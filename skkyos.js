...
module.exports = {
  ...
  fileExistsSync: function (filename, deleteIfExists) {
    try {
      ...
      if (deleteIfExists) fs.unlinkSync(filename)
    } catch (err) {
      if (err.code == 'ENOENT') {
  ...
  readTextFileSync: function (filename, replaceLineEnds) {
    ...
    try {
      if (fs.existsSync(filename)) {
        ...
        if (replaceLineEnds) s = skky.nonNull(s).replace(/\n$/, '')
      } else {
  ...
  getOsInfo: function () {
    if (skky.isNullOrUndefined(this.osinfo)) this.osinfo = this.getOsInfoRaw()
  ...
  getOsInfoRaw: function () {
    ...
    if (distro.length > 0) {
      ...
      if (distro.toLowerCase().indexOf('fedora') > -1) osinfo.isFedora = true
    }
  ...
  updateConfigSettings: function () {
    ...
    filesize = this.saveJsonToFileSync(
      config,
      skky.constants.ConfigFilenameSafeMode
    )
    if (filesize > 100) isok = true
